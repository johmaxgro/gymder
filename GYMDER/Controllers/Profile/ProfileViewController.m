...
//
//  Used for own profile and other users profile...
//  TODO (in OwnUserData and OtherUserData): Fusion between both to reduce redundancy

// TODO: Divide some initial settings (e.g. in viewdidlaod) into methods --> more structured
...
- (void)viewDidLoad {
    ...
    if([_ownUserData objectForKey:@"pows"] != nil){
//        power = (int)[[_ownUserData objectForKey:@"powpeople"]count];
        power = [[_ownUserData objectForKey:@"pows"] intValue];
        NSLog(@"POWS of visited user: %@", [_ownUserData objectForKey:@"pows"]);
    }else{
        power = 0;
    }
...
-(void)powedOrUnpowed{
    if([[_ownUserData objectForKey:@"powpeople"] containsObject:[NSString stringWithFormat:@"%lu", [QBSession currentSession].currentUser.ID]]){
        NSLog(@"Current user is powing visited person.");
...
-(void)updatePowing:(int)action{
    ...
    [QBRequest objectsWithClassName:@"UserProperties" extendedRequest:request successBlock:^(QBResponse *response, NSArray *objects, QBResponsePage *page) {
        ...
        [self updatePowingOnDatabase:powpeople withID:object.ID andAction:action];
...
-(void)updatePowingOnDatabase:(NSMutableArray*)toUpdate withID:(NSString *)objectID andAction:(int)action{
    ...
    if(action == 1){
        ...
        [updateObject.fields setObject:@(power+1) forKey:@"pows"];
        powed = YES;
        [_powButton setImage:[UIImage imageNamed:@"PROFILE POW SELECTED.png"] forState:UIControlStateNormal];
